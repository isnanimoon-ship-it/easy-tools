import { ArrowRight, Binary, Braces, FileKey, FileText, Globe2, ImageIcon, Link2, Palette, QrCode, ShieldCheck, Sparkles } from "lucide-react";
      ...
      <section className="border-b border-slate-200 bg-white">
        <Container className="py-16 sm:py-24">
          <div className="max-w-3xl">
            ...
            <h1 className="whitespace-pre-line text-4xl font-bold tracking-tight text-slate-950 sm:text-6xl">
              {t("title")}
            </h1>
      ...
      <section aria-labelledby="tools-heading">
        <Container className="py-12 sm:py-16">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div><h2 id="tools-heading" className="text-2xl font-bold text-slate-950">{t("tools.title")}</h2><p className="mt-2 text-slate-600">{t("tools.description")}</p></div>
            <p className="flex items-center gap-2 text-sm font-semibold text-slate-500"><ShieldCheck aria-hidden="true" size={18} />{t("tools.privacy")}</p>
          </div>
...
function ToolCard({ href, icon, title, description, action }: { href: "/tools/word-counter" | "/tools/json-formatter" | "/tools/password-generator" | "/tools/base64-converter" | "/tools/url-encoder-decoder" | "/tools/youtube-thumbnail-downloader" | "/tools/qr-code-generator" | "/tools/ip-info" | "/tools/image-color-picker"; icon: React.ReactNode; title: string; description: string; action: string }) {
  return <Link href={href} className="group rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition hover:border-blue-300 hover:shadow-md focus:outline-none focus:ring-4 focus:ring-blue-100">
    <span className="grid size-11 place-items-center rounded-xl bg-blue-50 text-blue-700">{icon}</span>
    <h3 className="mt-5 text-xl font-bold text-slate-950">{title}</h3>
    <p className="mt-2 leading-7 text-slate-600">{description}</p>
    <span className="mt-5 inline-flex items-center gap-2 font-semibold text-blue-700">{action}<ArrowRight aria-hidden="true" size={18} className="transition-transform group-hover:translate-x-1" /></span>
  </Link>;
}
