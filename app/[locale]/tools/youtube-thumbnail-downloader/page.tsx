...
function Info({ title, text, blue = false }: { title: string; text: string; blue?: boolean }) {
  return <div className={`rounded-2xl border p-5 sm:p-6 ${blue ? "border-[var(--info-border)] bg-[var(--info-bg)]" : "border-[var(--border)] bg-[var(--surface)]"}`}><h2 className={`font-bold ${blue ? "text-[var(--info-fg)]" : "text-[var(--foreground)]"}`}>{title}</h2><p className={`mt-2 text-sm leading-6 ${blue ? "text-[var(--info-fg)]" : "text-[var(--text-muted)]"}`}>{text}</p></div>;
}
