...
import { ScreenshotStatusbarRemover } from "@/components/tools/screenshot-statusbar-remover/screenshot-statusbar-remover";
import { routing, type AppLocale } from "@/i18n/routing";
import { createPageMetadata } from "@/lib/seo";
import { REPRESENTATIVE_TOOL_DETAILS } from "@/lib/tools/representative-detail-data";

type Props = { params: Promise<{ locale: AppLocale }> };
const path = "/tools/screenshot-statusbar-remover" as const;

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { locale } = await params;
  if (!hasLocale(routing.locales, locale)) notFound();
  const t = await getTranslations({ locale, namespace: "Tools.screenshotStatusbarRemover.metadata" });
...
export default async function Page({ params }: Props) {
  ...
  const t = await getTranslations("Tools.screenshotStatusbarRemover");
