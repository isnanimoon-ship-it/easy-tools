...
export default async function Page({ params }: PageProps) {
  ...
  const labels: IpInfoLabels = {
    ...
    lookup: t("actions.lookup"), refresh: t("actions.refresh"), copy: t("actions.copy"), copied: t("actions.copied"), unknown: t("result.unknown"), approximate: t("result.approximate"), retryAfter: t("errors.retryAfter"),
  ...
  return <>
    <section className="border-b border-slate-200 bg-white"><Container className="py-10 sm:py-14"><div className="max-w-3xl"><h1 className="text-3xl font-bold tracking-tight text-slate-950 sm:text-5xl">{t("title")}</h1><p className="mt-4 text-lg leading-8 text-slate-600">{t("description")}</p><p className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-950">{t("provider.notice")} <a href="https://ipwhois.io/privacy" target="_blank" rel="noreferrer" className="font-semibold underline underline-offset-2">{t("provider.privacyLink")}</a></p></div></Container></section>
