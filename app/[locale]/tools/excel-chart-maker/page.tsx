...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return createPageMetadata({ locale, title: t("title"), description: t("description"), pathname: `/${locale}/tools/excel-chart-maker` });
}
...
export default async function Page({ params }: PageProps) {
  ...
  return <><section className="border-b border-[var(--border)] bg-[var(--surface)]"><Container className="py-10 sm:py-14"><div className="max-w-4xl"><h1 className="text-3xl font-bold tracking-tight sm:text-5xl">{t("title")}</h1><p className="mt-4 text-lg leading-8 text-[var(--text-muted)]">{t("description")}</p></div></Container></section><Container className="py-8 sm:py-12"><ExcelChartMaker/><section className="mt-8 grid gap-4 md:grid-cols-3"><Info title={t("guide.easyTitle")} text={t("guide.easyText")}/><Info title={t("guide.accurateTitle")} text={t("guide.accurateText")}/><Info title={t("guide.privateTitle")} text={t("guide.privateText")} blue/></section></Container></>;
}
