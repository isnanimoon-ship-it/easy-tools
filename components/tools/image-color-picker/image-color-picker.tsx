...
export function ImageColorPicker({labels}:{labels:ImageColorPickerLabels}) {
  ...
  return <section className="grid gap-6">
    <section className={`rounded-2xl border-2 border-dashed p-5 text-center transition sm:p-8 ${dragging?"border-[var(--primary)] bg-[var(--info-bg)]":"border-[var(--border)] bg-[var(--surface)]"}`} onDragEnter={e=>{e.preventDefault();dragDepth.current++;setDragging(true);}} onDragOver={e=>e.preventDefault()} onDragLeave={e=>{e.preventDefault();dragDepth.current--;if(dragDepth.current<=0){dragDepth.current=0;setDragging(false);}}} onDrop={e=>{e.preventDefault();dragDepth.current=0;setDragging(false);acceptFiles(e.dataTransfer.files);}} aria-busy={busy}>
      <ImagePlus aria-hidden="true" className="mx-auto text-[var(--primary)]" size={34}/><h2 className="mt-3 text-xl font-bold text-[var(--foreground)]">{labels.uploadTitle}</h2><p className="mt-2 text-sm leading-6 text-[var(--text-muted)]">{dragging?labels.drop:labels.uploadHelp}</p>
      ...
      <div className="mt-4 flex flex-wrap justify-center gap-3"><label htmlFor="image-color-file" className="inline-flex min-h-11 cursor-pointer items-center justify-center gap-2 rounded-xl bg-[var(--primary-fill)] px-4 py-2.5 font-semibold text-white hover:bg-[var(--primary-fill-hover)] focus-within:ring-4 focus-within:ring-[var(--focus-ring)]"><Upload aria-hidden="true" size={18}/>{loaded?labels.change:labels.choose}</label>{loaded?<Button variant="secondary" onClick={reset}><RotateCcw aria-hidden="true" size={18}/>{labels.reset}</Button>:null}</div>
      {busy?<p role="status" className="mt-4 font-semibold text-[var(--info-fg)]">{labels.loading}</p>:null}{error?<p role="alert" className="mt-4 rounded-xl border border-[var(--error-border)] bg-[var(--error-bg)] p-3 text-sm font-medium text-[var(--error-fg)]">{labels.errors[error]}</p>:null}
    </section>
    {loaded?<>
      <section className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4 shadow-sm sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div className="min-w-0"><h2 className="font-bold text-[var(--foreground)]">{labels.imageInfo}</h2><dl className="mt-3 grid gap-2 text-sm sm:grid-cols-2"><Info name={labels.fileName} value={loaded.name}/><Info name={labels.format} value={loaded.format}/><Info name={labels.dimensions} value={`${loaded.width} × ${loaded.height}px`}/><Info name={labels.fileSize} value={formatFileSize(loaded.size)}/></dl></div><label className="font-bold text-[var(--foreground)]">{labels.zoom}<select value={zoom} onChange={e=>setZoom(e.target.value as typeof zoom)} className="mt-2 block min-h-11 rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3"><option value="fit">{labels.fit}</option>{zooms.slice(1).map(item=><option key={item} value={item}>{item}%</option>)}</select></label></div></section>
      <div className="grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(20rem,0.65fr)]">
        <section className="min-w-0 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4 shadow-sm sm:p-6"><h2 className="font-bold text-[var(--foreground)]">{labels.preview}</h2><p className="mt-1 text-sm text-[var(--text-muted)]">{labels.selectHelp}</p><div className="mt-4 max-h-[70vh] min-h-56 max-w-full overflow-auto rounded-xl bg-[linear-gradient(45deg,#e2e8f0_25%,transparent_25%),linear-gradient(-45deg,#e2e8f0_25%,transparent_25%),linear-gradient(45deg,transparent_75%,#e2e8f0_75%),linear-gradient(-45deg,transparent_75%,#e2e8f0_75%)] bg-[length:20px_20px] bg-[position:0_0,0_10px,10px_-10px,-10px_0px] p-2"><div className="relative w-max max-w-full"><img ref={imageRef} src={loaded.url} alt={labels.previewAlt} draggable={false} onDragStart={e=>e.preventDefault()} onLoad={()=>setReady(true)} onClick={pick} onPointerMove={moveMagnifier} onPointerLeave={()=>setMagnifier(null)} style={imageStyle} className="block cursor-crosshair select-none"/>{selection?<span aria-hidden="true" className="pointer-events-none absolute size-5 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-white shadow-[0_0_0_2px_#0f172a]" style={{left:`${(selection.x+.5)/loaded.width*100}%`,top:`${(selection.y+.5)/loaded.height*100}%`}}/>:null}</div></div></section>
        <section className="min-w-0 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4 shadow-sm sm:p-6"><h2 className="font-bold text-[var(--foreground)]">{labels.coordinateTitle}</h2><div className="mt-3 grid grid-cols-2 gap-3"><NumberField label={labels.x} value={xInput} set={setXInput} max={loaded.width-1}/><NumberField label={labels.y} value={yInput} set={setYInput} max={loaded.height-1}/></div><Button className="mt-3 w-full" onClick={selectCoordinate}>{labels.selectCoordinate}</Button>
          <h2 className="mt-6 font-bold text-[var(--foreground)]">{labels.selected}</h2>{selection?<Result selection={selection} labels={labels} copied={copied} copy={copy}/>:<p className="mt-3 rounded-xl bg-[var(--surface-muted)] p-4 text-sm text-[var(--text-muted)]">{labels.noSelection}</p>}
        </section>
      </div>
      <section className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4 shadow-sm sm:p-6"><h2 className="font-bold text-[var(--foreground)]">{labels.history}</h2>{history.length?<div className="mt-4 grid grid-cols-3 gap-3 sm:grid-cols-6 lg:grid-cols-12">{history.map((item,index)=><button key={`${item.formats.hex}-${item.x}-${item.y}-${index}`} type="button" onClick={()=>{setSelection(item);setXInput(String(item.x));setYInput(String(item.y));}} aria-label={`${item.formats.hex}, X ${item.x}, Y ${item.y}`} className="min-w-0 rounded-xl border border-[var(--border)] p-2 text-xs font-bold focus:ring-4 focus:ring-[var(--focus-ring)]"><span className="block aspect-square rounded-lg border border-[var(--border)]" style={{backgroundColor:`rgba(${item.rgba.r},${item.rgba.g},${item.rgba.b},${item.rgba.a/255})`}}/><span className="mt-1 block truncate">{item.formats.hex}</span></button>)}</div>:<p className="mt-2 text-sm text-[var(--text-muted)]">{labels.historyEmpty}</p>}</section>
      <canvas ref={canvasRef} width={1} height={1} className="hidden"/>
      <div data-testid="pixel-magnifier" aria-hidden="true" className={`pointer-events-none fixed z-50 overflow-hidden rounded-xl border-4 border-white bg-[var(--surface)] shadow-2xl transition-opacity ${magnifier?"opacity-100":"opacity-0"}`} style={{left:magnifier?.left??0,top:magnifier?.top??0}}><div className="relative size-[220px] overflow-hidden bg-[linear-gradient(45deg,#e2e8f0_25%,transparent_25%),linear-gradient(-45deg,#e2e8f0_25%,transparent_25%),linear-gradient(45deg,transparent_75%,#e2e8f0_75%),linear-gradient(-45deg,transparent_75%,#e2e8f0_75%)] bg-[length:20px_20px]"><img data-testid="magnified-image" src={loaded.url} alt="" className="absolute max-w-none [image-rendering:pixelated]" style={{left:`${(5-(magnifier?.x??0))*20}px`,top:`${(5-(magnifier?.y??0))*20}px`,width:`${loaded.width*20}px`,height:`${loaded.height*20}px`}}/><span className="absolute left-[100px] top-[100px] size-5 border-2 border-red-500"/><span className="absolute inset-0 bg-[linear-gradient(to_right,rgba(255,255,255,.28)_1px,transparent_1px),linear-gradient(to_bottom,rgba(255,255,255,.28)_1px,transparent_1px)] bg-[size:20px_20px]"/></div><div className="bg-[var(--code-bg)] px-3 py-1.5 text-center font-mono text-xs font-bold text-white">X {magnifier?.x??0} · Y {magnifier?.y??0}</div></div>
    </>:null}
    <p className="rounded-xl bg-[var(--info-bg)] p-4 text-sm leading-6 text-[var(--info-fg)]">{labels.privacy}</p>
  </section>;
}

function Info({name,value}:{name:string;value:string}){return <div className="min-w-0"><dt className="text-xs font-bold text-[var(--text-muted)]">{name}</dt><dd className="mt-0.5 break-words font-medium text-[var(--foreground)]">{value}</dd></div>}
function NumberField({label,value,set,max}:{label:string;value:string;set:(value:string)=>void;max:number}){return <label className="font-bold text-[var(--foreground)]">{label}<input type="number" min={0} max={max} step={1} value={value} onChange={e=>set(e.target.value)} className="mt-2 min-h-11 w-full rounded-xl border border-[var(--border)] px-3 font-mono focus:ring-4 focus:ring-[var(--focus-ring)]"/></label>}
function Result({selection,labels,copied,copy}:{selection:Selection;labels:ImageColorPickerLabels;copied:keyof ColorFormats|null;copy:(kind:keyof ColorFormats)=>void}){return <div className="mt-3"><div className="overflow-hidden rounded-xl border border-[var(--border)] bg-[linear-gradient(45deg,#e2e8f0_25%,transparent_25%),linear-gradient(-45deg,#e2e8f0_25%,transparent_25%),linear-gradient(45deg,transparent_75%,#e2e8f0_75%),linear-gradient(-45deg,transparent_75%,#e2e8f0_75%)] bg-[length:20px_20px]"><div className="h-28" style={{backgroundColor:`rgba(${selection.rgba.r},${selection.rgba.g},${selection.rgba.b},${selection.rgba.a/255})`}}/></div><p role="status" className="mt-2 text-sm font-semibold text-[var(--foreground)]">X {selection.x} · Y {selection.y} · Alpha {Math.round(selection.rgba.a/255*1000)/1000}</p><div className="mt-4 grid gap-3">{(Object.keys(selection.formats) as Array<keyof ColorFormats>).map(kind=><div key={kind} className="min-w-0 rounded-xl border border-[var(--border)] p-3"><p className="text-xs font-bold uppercase text-[var(--text-muted)]">{labels.formats[kind]}</p><div className="mt-1 flex min-w-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><code className="min-w-0 break-all text-sm font-semibold text-[var(--foreground)]">{selection.formats[kind]}</code><Button variant="secondary" aria-label={`${labels.formats[kind]} ${labels.copy}`} onClick={()=>copy(kind)}><Copy aria-hidden="true" size={16}/>{copied===kind?labels.copied:labels.copy}</Button></div></div>)}</div></div>}
