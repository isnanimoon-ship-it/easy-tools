...
const labels: QrCodeGeneratorLabels = {
  ...
  currentSize: "Size", currentLevel: "Level", currentMargin: "Margin", inputType: "Type", inputTypes: { text: "Text", url: "URL" },
...
const okRenderer: QrRenderer = vi.fn(async (canvas, input, options) => { canvas.width = options.size; canvas.height = options.size; return { ok: true as const, metadata: { version: input.length > 900 ? 25 : 1, moduleCount: 21, modulePixels: 8, warning: input.length > 900 } }; });
...
describe("QrCodeGenerator", () => {
  ...
  it("clears state, restores defaults and focus, and ignores stale results", async () => {
    ...
    finish({ ok: true, metadata: { version: 1, moduleCount: 21, modulePixels: 8, warning: false } }); await act(async () => Promise.resolve());
