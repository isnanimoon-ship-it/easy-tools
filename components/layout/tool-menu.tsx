...
export function ToolMenu() {
  ...
  return <div ref={rootRef} className="relative">
    <nav aria-label={t("label")} className="hidden items-center gap-1 xl:flex">
      {categories.slice(0, 3).map(category => <CategoryButton key={category.key} category={category} active={activeCategory === category.key} open={open === category.key} toggle={toggle} label={t(`categories.${category.key}`)}/>) }
      <button data-menu="all" type="button" aria-expanded={open === "all"} aria-controls="tool-panel" onClick={() => toggle("all")} className={`inline-flex min-h-11 items-center gap-2 rounded-xl px-3 py-2 text-sm font-bold transition focus:outline-none focus:ring-4 focus:ring-blue-100 ${open === "all" || activeCategory === "other" ? "bg-blue-50 text-blue-800" : "text-slate-700 hover:bg-slate-100 hover:text-slate-950"}`}><Grid2X2 aria-hidden="true" size={17}/>{t("all")}<ChevronDown aria-hidden="true" size={15} className={open === "all" ? "rotate-180 transition" : "transition"}/></button>
    </nav>
