...
try {
  for (const [locale, width, trigger, all] of mobileCases) {
    ...
    const registeredToolCount = await page.locator('main a[href*="/tools/"]').evaluateAll(nodes => new Set(nodes.map(node => node.getAttribute("href"))).size);
    ...
    assert.equal(await panel.getByRole("heading").count(), 6); await page.keyboard.press("Escape"); assert.equal(await panel.count(), 0);
    assert.equal(await page.getByRole("button", { name: trigger, exact: true }).evaluate(node => node === document.activeElement), true);
    await context.close();
  }

  const context = await browser.newContext({ viewport: { width: 1536, height: 850 } }); const page = await context.newPage(); watch(page, "ko/1536");
  const homeHtml = await (await context.request.get(`${baseUrl}/ko`)).text(); const registeredToolCount = new Set([...homeHtml.matchAll(/href="(\/ko\/tools\/[^"]+)"/g)].map(match => match[1])).size;
  await page.goto(`${baseUrl}/ko/tools/json-formatter`, { waitUntil: "domcontentloaded" }); await assertLayout(page);
  for (const label of ["이미지", "텍스트", "개발자", "생성기", "파일·데이터", "네트워크", "전체 도구"]) assert.equal(await page.getByRole("button", { name: new RegExp(label) }).isVisible(), true);
  ...
  await developer.click(); let panel = page.locator("#tool-panel"); assert.equal(await panel.getByRole("link").count(), 7); assert.equal(await panel.getByRole("heading", { name: "개발자" }).count(), 1);
  await page.keyboard.press("Escape"); assert.equal(await developer.evaluate(node => node === document.activeElement), true);
  await page.getByRole("button", { name: /전체 도구/ }).click(); panel = page.locator("#tool-panel"); assert.equal(await panel.getByRole("link").count(), registeredToolCount); assert.equal(await panel.getByRole("heading").count(), 6);
  await page.screenshot({ path: "artifacts/tool-menu-1536-ko.png" });
  await context.close();

  assert.deepEqual(consoleErrors, []); assert.deepEqual(pageErrors, []);
  process.stdout.write(JSON.stringify({ mobileCases: mobileCases.length, desktopMenus: 7, desktopCategories: 6, links: registeredToolCount, consoleErrors: 0, pageErrors: 0, horizontalOverflow: 0, overlap: 0 }, null, 2));
