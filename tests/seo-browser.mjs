...
try{
  ...
  const sitemap=await (await context.request.get(`${baseUrl}/sitemap.xml`)).text();assert.equal((sitemap.match(/<url>/g)??[]).length,39);assert.equal((sitemap.match(/hreflang="x-default"/g)??[]).length,39);assert.match(sitemap,/https:\/\/www\.konly\.co\.kr\/ja\/tools\/cron-expression-generator/);
  ...
  assert.deepEqual(consoleErrors,[]);assert.deepEqual(pageErrors,[]);process.stdout.write(JSON.stringify({sitemapUrls:39,hreflangSets:39,locales:3,canonical:true,openGraph:true,twitter:true,jsonLd:["WebSite","WebApplication"],ogImage:"PNG",consoleErrors:0,pageErrors:0},null,2));
