...
try{
  ...
  for(const [locale,path] of [["ko","word-counter"],["en","json-formatter"],["ja","image-color-picker"],["ko","screenshot-stitcher"],["en","privacy-redactor"],["ko","excel-chart-maker"],["ja","p2p-file-transfer"],["ko","markdown-viewer"]]){
    ...
    const structuredData=(await page.locator('script[type="application/ld+json"]').allTextContents()).map(JSON.parse);const data=structuredData.find(item=>item["@graph"]);assert.deepEqual(data["@graph"].map(item=>item["@type"]),["WebSite","WebApplication"]);assert.equal(data["@graph"][0].url,"https://www.konly.co.kr");
  }
  for(const slug of ["about","privacy","contact","terms"]){
    for(const locale of ["ko","en","ja"]){
      const pathname=`/${locale}/${slug}`;await page.goto(`${baseUrl}${pathname}`,{waitUntil:"networkidle"});
      assert.equal(await page.locator('link[rel="canonical"]').getAttribute("href"),`https://www.konly.co.kr${pathname}`);
      for(const alternate of ["ko","en","ja","x-default"])assert.equal(await page.locator(`link[rel="alternate"][hreflang="${alternate}"]`).count(),1);
      if (["privacy","contact"].includes(slug)) assert.equal((await page.getByRole("link",{name:/isnanik@daum\.net/}).getAttribute("href")),"mailto:isnanik@daum.net");
    }
  }
