...
try{
  ...
  const robots=await (await context.request.get(`${baseUrl}/robots.txt`)).text();assert.match(robots,/User-Agent: \*/i);assert.match(robots,/Allow: \//i);assert.match(robots,/Sitemap: https:\/\/konly\.co\.kr\/sitemap\.xml/i);
  const sitemap=await (await context.request.get(`${baseUrl}/sitemap.xml`)).text();assert.equal((sitemap.match(/<url>/g)??[]).length,30);assert.equal((sitemap.match(/hreflang="x-default"/g)??[]).length,30);assert.match(sitemap,/https:\/\/konly\.co\.kr\/ja\/tools\/image-color-picker/);
  for(const [locale,path] of [["ko","word-counter"],["en","json-formatter"],["ja","image-color-picker"]]){
    ...
    assert.equal(await page.locator('link[rel="canonical"]').getAttribute("href"),`https://konly.co.kr${pathname}`);
    for(const alternate of ["ko","en","ja","x-default"])assert.equal(await page.locator(`link[rel="alternate"][hreflang="${alternate}"]`).count(),1);
    assert.equal(await page.locator('meta[property="og:url"]').getAttribute("content"),`https://konly.co.kr${pathname}`);
    assert.equal(await page.locator('meta[property="og:image"]').getAttribute("content"),"https://konly.co.kr/og");
    assert.equal(await page.locator('meta[name="twitter:card"]').getAttribute("content"),"summary_large_image");
    assert.ok((await page.locator('meta[property="og:title"]').getAttribute("content"))?.length>3);
    const data=JSON.parse(await page.locator('script[type="application/ld+json"]').textContent());assert.deepEqual(data["@graph"].map(item=>item["@type"]),["WebSite","WebApplication"]);assert.equal(data["@graph"][0].url,"https://konly.co.kr");
  }
